...
static class Config
{
    ...
    [
        ...
        new("google-tasks", "Google Tasks",
        [
            "The Google Tasks section uses the official Tasks API (OAuth). One-time setup:",
            ...
            "   then set Publishing status to 'In production' (unverified is fine for",
            "   personal use — while it stays in 'Testing', Google expires the sign-in",
            "   every 7 days).",
            "4. Credentials > Create Credentials > OAuth client ID > Desktop app.",
            "Then fill in here — Line 1: the Client ID. Line 2: the Client Secret.",
            "The first visit to the section opens a browser sign-in once; tokens are",
            "kept in data/google-tasks-token.json and refresh themselves from then on.",
            "Note: due dates only — Google's API drops times of day in both directions,",
            "so tasks show and take dates without times here.",
        ], false),
        ...
        [
            ...
            "3. Enable the spark:all and spark:kms scopes (spark:kms lets the app read the",
            "   emoji reactions on messages, which are end-to-end encrypted).",
        ...
        new("paylocity", "Paylocity time clock",
        [
            "The Time clock section drives access.paylocity.com in an embedded browser.",
            "Paylocity ends its web session when the browser closes, so the app signs in",
            "fresh on every visit. Always needed:",
            "  company = your Paylocity company ID",
            "With SSO (sign-in through the employer's identity provider) nothing else is",
            "needed: the first visit opens a browser window to sign in once — check 'Stay",
            "signed in' there — and later visits sign in silently.",
            "Companies using Paylocity's own login instead also add:",
            "  username = ...  and  password = ...",
            "(stored in plain text — keep this folder private).",
            "Optional: url = ...       a different portal/SSO entry URL",
            "Optional: timeurl = ...   a different Time & Labor page (default is",
            "                          webtime2.paylocity.com/WebTime/v3/Employee)",
        ], false),
        ...
        [
            ...
            "  header = on|off        keep the time/date and weather at the top of every",
            "                         screen (menus, pager, readers), not just the main menu",
            ...
            "                         (also the background notification poll rate)",
            "  notify = on|off        background notifications: new email, texts, Webex,",
            "                         and Discord activity show a count on the header's",
            "                         clock line; press N in any menu or view to open",
            "                         them and jump straight to the message",
            "  notify-email = on|off    per-source switches for the notification poll.",
            "  notify-texts = on|off    Email only counts mail newer than your last look",
            "  notify-webex = on|off    at that inbox; texts need the phone pairing from",
            "  notify-discord = on|off  the Texts section and keep a hidden browser open;",
            "                         Discord polls at most every 5 minutes, and servers",
            "                         only flag @mentions (DMs flag any unread)",
